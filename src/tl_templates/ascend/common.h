...
template <>
struct CopyL0CToGmQuantMode<float, float> {
    static constexpr auto VALUE = QuantMode_t::NoQuant;
};

// CopyL0ToGm output int32
template <>
struct CopyL0CToGmQuantMode<int32_t, int32_t> {
    static constexpr auto VALUE = QuantMode_t::NoQuant;
};

// CopyL0ToGm cast int32_t to fp16
template <>
struct CopyL0CToGmQuantMode<int32_t, half> {
...
CATLASS_DEVICE void copy_l0c_to_gm(GlobalTensor<T2> dstTensor,
  ...
  AscendC::Fixpipe<T2, T1, AscendC::CFG_ROW_MAJOR>(dstTensor, srcTensor, {srcN, srcM, srcM, dstN, enRelu, quantMode, 0, 1, 0, 0, false});
}
...
CATLASS_DEVICE void copy_gm_to_ub(LocalTensor<T> dstTensor,
                                  GlobalTensor<T> srcTensor, uint32_t realSrcN = srcN) {
  AscendC::DataCopyExtParams dataCopyParams(
      dstM,
      dstN * sizeof(T),
      (realSrcN - dstN) * sizeof(T),
      0,
      0
  );
...
template<typename T>
CATLASS_DEVICE void MergeSort(const LocalTensor<T> &dst, const LocalTensor<T> &src, uint32_t blockSize, uint32_t block_num, uint32_t is_copy)
{
  // 初始化合并排序参数
...
template<typename T>
CATLASS_DEVICE void TopK(const LocalTensor<T> &dst, const LocalTensor<T> &src, const LocalTensor<T> &tmp, uint32_t blockSize)
{
  // 初始化合并排序参数
  AscendC::MrgSort4Info params;
  params.elementLengths[0] = blockSize;
...
template<typename T>
CATLASS_DEVICE void GatherMask(const LocalTensor<T> &dst, const LocalTensor<T> &sortedTensor, int64_t extractNum)
{
    ...
    GatherMask(dst.template ReinterpretCast<uint32_t>(), sortedTensor.template ReinterpretCast<uint32_t>(), src1Pattern, false, static_cast<uint32_t>(0), gatherMaskParams, rsvdCnt);
    PipeBarrier<PIPE_V>();
}

template<typename T>
CATLASS_DEVICE void InitSortBuf(const LocalTensor<T> &src, int64_t eleNum, int64_t rsv = 0)
{
...
template <typename T1, typename T2, uint32_t L1_block_M, uint32_t L1_block_N, uint32_t L1_block_K, uint32_t BLOCK_M, uint32_t BLOCK_N, uint32_t BLOCK_K, bool transpose_A=false, bool transpose_B=false>
CATLASS_DEVICE void gemm_v1(
  LocalTensor<T1> const &A,
  LocalTensor<T1> const &B,
  LocalTensor<T2> const &C, // this must be located in l0c
  AscendC::TBuf<AscendC::TPosition::A2> &l0a_,
  ...
  if constexpr(!transpose_A) {
    tl::ascend::copy_l1_to_l0a<half, layout::zN, L1_block_M, L1_block_K, BLOCK_M, BLOCK_K>(l0a, A);
  } else {
    tl::ascend::copy_l1_to_l0a<half, layout::nZ, L1_block_M, L1_block_K, BLOCK_M, BLOCK_K>(l0a, A);
  }

  if constexpr(!transpose_B) {
    tl::ascend::copy_l1_to_l0b<half, layout::zN, L1_block_K, L1_block_N, BLOCK_K, BLOCK_N>(l0b, B);
  } else {
    tl::ascend::copy_l1_to_l0b<half, layout::nZ, L1_block_K, L1_block_N, BLOCK_K, BLOCK_N>(l0b, B);
  }

  AscendC::PipeBarrier<PIPE_ALL>();
  tl:ascend::mma<T1, T2, BLOCK_M, BLOCK_N, BLOCK_K>(l0a, l0b, C, clear);
  AscendC::PipeBarrier<PIPE_ALL>();
}

template <typename T>
CATLASS_DEVICE void brcb(const LocalTensor<T>& dst,
                         const LocalTensor<T>& src0,
                         const uint8_t repeatTime,
                         const uint16_t dstBlkStride,
...
template <typename T1, typename T2, typename LayOutL1, typename LayoutGM,
          ...
          uint32_t baseK, bool init, bool is_transpose_A = false, bool is_transpose_B = false,
          bool enable_relu = false>
CATLASS_DEVICE void gemmL1(LocalTensor<T1> A, LocalTensor<T1> B, GlobalTensor<T1> C,
                           LocalTensor<T1> A2, LocalTensor<T1> B2, LocalTensor<T2> C2) {
  for (uint32_t loopM = 0; loopM < M / baseM; loopM++) {
    ...
    for (uint32_t loopN = 0; loopN < N / baseN; loopN++) {
      copy_l1_to_l0b<T1, LayOutL1, K, N, baseK, baseN>(B2, B[loopN * baseN * K]);
      ...
      copy_l0c_to_gm<T1, T2, LayoutGM, baseM, baseN, M, N>(C[loopM * baseM * N + loopN * baseN], C2, enable_relu);
