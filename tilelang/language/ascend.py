...
def gemm_v1(A, B, C, transpose_A=False, transpose_B=False, init=False):
    ...
    # assert _dtype(A) == _dtype(B), f"gemm A and B dtype mismatch: {_dtype(A)} vs {_dtype(B)}"
    return T.call_extern(
        "handle", f"tl::ascend::gemm_v1<{_dtype(A)}, {_dtype(C)}, {L1_BLOCK_M}, {L1_BLOCK_N}, {L1_BLOCK_K}, {BLOCK_M}, {BLOCK_N}, {L1_BLOCK_K}, {str(transpose_A).lower()}, {str(transpose_B).lower()}>",
        Aptr, Bptr, Cptr, init)
...
def fill(buffer: Buffer, value: PrimExpr):
    ...
    return T.call_extern("handle", f"AscendC::Duplicate<{_dtype(buffer)}>", buffer.access_ptr("w"), value, size)


def arith_progression(buffer: Buffer, first_value, diff_value, count):
    return T.call_extern("handle", f"AscendC::ArithProgression<{_dtype(buffer)}>", buffer.access_ptr("w"), first_value, diff_value, count)
...
def merge_sort(dst: Buffer, src: Buffer, block_size, block_num, is_copy):
    return T.call_extern("handle", f"tl::ascend::MergeSort<{_dtype(dst)}>", dst.access_ptr("w"), src.access_ptr("r"), block_size, block_num, is_copy)


def topk(dst: Buffer, src: Buffer, tmp: Buffer, block_size):
    return T.call_extern("handle", f"tl::ascend::TopK<{_dtype(dst)}>", dst.access_ptr("w"), src.access_ptr("r"), tmp.access_ptr("r"), block_size)


def gather_mask(dst: Buffer, src: Buffer, num):
    return T.call_extern("handle", f"tl::ascend::GatherMask<{_dtype(dst)}>", dst.access_ptr("w"), src.access_ptr("r"), num)


def init_sort_buf(buffer: Buffer, num, rsv):
    pass
    return T.call_extern("handle", f"tl::ascend::InitSortBuf<{_dtype(buffer)}>", buffer.access_ptr("w"), rsv, num)
